...
def test_lr_calc():
    """ Tests learning rate calculation is as expected"""
...
def test_loss_calc():
    "Tests loss calc is as expected"""
    np.random.rand(1)
    n_time = 20
    n_markets = 2
    n_batch = 1
    ...
    nn_pos = neural._positions_np(batch_in=batch_in)
...
def test_training_fakedata():
    """ Tests learning using training, should find the stock with exponential increases"""
    np.random.rand(1)
    n_time = 50
    n_markets = 3
    n_batch = 1
